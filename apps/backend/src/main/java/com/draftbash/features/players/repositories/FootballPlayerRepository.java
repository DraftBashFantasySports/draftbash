...
public class FootballPlayerRepository implements IFootballPlayerRepository {
    ...
    public Map<String, Object> getPlayer(int playerId) {
        final String SQL = """
            SELECT p.id, p.first_name, p.last_name, p.age, p.height, p.weight, p.injury_status,
                ...
                tdp.blocked_kicks, tdp.points_against, tdp.fantasy_points AS defense_fantasy_points,
                pn.headline, pn.summary, pn.analysis, pn.fantasy_outlook, pn.url, pn.date
            ...
            LEFT JOIN player_news AS pn ON p.id = pn.player_id
        ...
        db.query(SQL, params, (rowSets) -> {
            FootballPlayerDTO player = new FootballPlayerDTO(
                    rowSets.getInt("id"),
            ...
            PlayerNewsDTO news = new PlayerNewsDTO(
                    rowSets.getInt("id"),
                    rowSets.getString("headline"),
                    rowSets.getString("summary"),
                    rowSets.getString("analysis"),
                    rowSets.getString("fantasy_outlook"),
                    rowSets.getString("url"),
                    rowSets.getString("date"));
            playerInfo.put("player", player);
            playerInfo.put("news", news);
        });
        return playerInfo;
    }
    ...
    public void updatePlayerInjury(int playerId, String injuryStatus) {
        final String SQL = """
            UPDATE player
            SET injury_status = :injury_status::injury_status
            WHERE id = :player_id
            """;

        Map<String, Object> params = new HashMap<>();
        params.put("player_id", playerId);
        params.put("injury_status", injuryStatus);
